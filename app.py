...
def track_user_activity():
    ...
    POST actions are logged by their individual routes so the
    admin panel can show the real action instead of a generic
    Page visit.
    """
...
def edit_plan(plan_id):
    ...
    if request.method == "POST":
        ...
        # Allow the actual studied hours to be edited here too.
        try:
        ...
        # Studied hours cannot be greater than planned hours.
        if studied_hours > planned_hours:
...
def performance_prediction_page():
    ...
    total_hours = sum(
        float(plan["hours"] or 0)
        for plan in plans
    )

    studied_hours = sum(
        float(plan["studied_hours"] or 0)
        for plan in plans
    )
...
def profile_page():
    ...
    cursor.execute("""
        SELECT hours, studied_hours, status
        FROM study_plans
        WHERE user_id = ?
    """, (user_id,))
    ...
    completed_plans = sum(
        1
        for p in plans
        if str(p["status"] or "").lower() == "completed"
    )

    if total_hours > 0:
        progress = round(
            (studied_hours / total_hours) * 100
        )
    else:
        progress = 0
...
def progress_page():
    ...
    cursor.execute("""
        SELECT subject, hours, studied_hours, status
        FROM study_plans
        WHERE user_id = ?
        ORDER BY date DESC, id DESC
    """, (user_id,))
    ...
    pending = len(plans) - completed

    subject_progress = []

    for p in plans:

        planned = float(p["hours"] or 0)
        studied = float(p["studied_hours"] or 0)
